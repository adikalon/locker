# Locker
**hellpers/locker** - Предотвращает повторный запуск скрипта.

## Установка:
	composer require hellpers/locker
...
| Locker::set() принимает 5 необязательных параметра:
|
...
| умолчанию - true);
| 5. (bool) Обрабатывать сигналы - да/нет, true/false (только для unix-подобных
| ОС). Необходимо для того, чтобы lock-файл удалялся, когда скрипт убивается
| извне. Будет правильно работать только если в начале кода указать
| declare(ticks = 1) (по умолчанию - false);
|
...
Locker::set(__DIR__, 'test.lock', false, true, true);
